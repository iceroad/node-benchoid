...
agentSpawnInterval=${agentSpawnInterval}`)
  ...
  _.forEach(_.range(0, numAgents), idx => {
    ...
    _.delay(() => {
      ...
      const args = ['.', '--lifetime', agentLifetime]
      console.error(`${pid}: spawning agent ${idx}: agendId=${agentId}, args="${args.join(' ')}"`)
      ...
      //
      // Force-kill agents after their lifetimes.
      //
      setTimeout(() => {
      ...
      //
      child.once('exit', () => {
        console.error(`${pid}: agent ${idx} (${agentId}) died.`)
        child.isDead = true
