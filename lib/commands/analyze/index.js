...
function AnalyzeRun(runName, cb) {
  console.log(col.starting(`Analyzing run "${col.bold(runName)}"`));
  const analysisModules = require(path.resolve('analysis'));
  const dataDir = path.resolve('run_data', runName);
  ...
  return async.auto({
    ...
    analysis: ['rows', (deps, cb) => {
      ...
      const dataFileStr = `window.DATA = ${json(analysis, null, 2)};\n`;
...
function AnalyzeCmd(args) {
  const runners = [];
  if (args.all) {
    const runs = getRunDataIndex();
    _.forEach(runs, run => runners.push((cb) => AnalyzeRun(run.name, cb)));
  } else {
    assert(args.runName, 'Please specify --runName');
    runners.push((cb) => AnalyzeRun(args.runName, cb));
  }

  async.series(runners, (err) => {
    if (err) {
