...
function run(args) {
  ...
  walkthrough(args).then((expConfig) => {
    ...
    let rowsWritten = 0, bytesWritten = 0;

    //
    // Parallel execute drivers.
    //
    const runners = _.map(clientTier, remoteIp => (cb) => {
      const cmdLine = json(`node --max_old_space_size=8192 driver \
...
--steady ${expConfig.steady}`);
      ...
      child.once('exit', (code, signal) => {
        if (code || signal) {
          return cb(new Error(
            `benchoid: local child for ${remoteIp} died with code=${code}, signal={$signal}.`));
        }
        return cb();
      });
