...
function getClientTier() {
  const cmdLine = 'terraform output'
  ...
  if (rv.status !== 0) {
    throw new Error(`Cannot run "terraform output": ${rv.stderr.toString('utf-8')}`)
  }
...
function getVersionSync() {
  return exec.shellSync('terraform version').match(/Terraform v(\S+)/)[1]
}
