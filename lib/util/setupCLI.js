...
`
  ...
  const firstColWidth = _.max(_.map(usageTableRows, row => stripAnsi(row[0]).length)) + 1
...
function ParseArguments(cmd, args) {
  ...
  //
  const allAliases = _.fromPairs(_.map(_.flatten(_.map(argSpec, 'flags')), flagAlias => [flagAlias, true]))
  ...
  _.forEach(args, (argVal, argKey) => {
    ...
    if (!(argKey in allAliases)) {
      ...
      throw new Error(`Unknown command-line option "${col.bold(d + argKey)}" specified ` +
  ...
  _.forEach(argSpec, (aspec, idx) => {
    ...
    const foundAlias = _.find(aspec.flags, flagAlias => flagAlias && (flagAlias in args))
